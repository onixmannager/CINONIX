...
window.registrarUsuario = async function(email, password) {
  try {
    ...
    await setDoc(doc(db, "usuarios", user.uid), {
      ...
      dineroAcumulado: 0,
      // ✅ FIX 2: Campo renombrado de "referidoPor" a "referredBy" para que
      //            coincida con la query del dashboard (where("referredBy", "==", ...))
      referredBy: codigoReferido || null,
  ...
  if (user) {
    try {
      ...
      const updateData = {
        ...
        afiliado: true,
        // ✅ FIX 3: Eliminada la regeneración del codigoAfiliado.
        //            Si se regenera aquí, cualquier link compartido antes de pagar
        //            deja de funcionar. El código generado en el registro se mantiene.
